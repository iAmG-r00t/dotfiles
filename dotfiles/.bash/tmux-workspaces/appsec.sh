...
then
	...
	tmux send-keys -t 'ADB' 'cd /opt/genymobile/genymotion/tools' C-m 'clear' C-m

	tmux new-window -t $session -n 'Burp'
	tmux send-keys -t 'Burp' 'tools' C-m

	tmux new-window -t $session -n 'Frida'
	tmux send-keys -t 'Frida' 'cd /home/$USER/Gitools/AppSecTools/frida-scripts' C-m 'clear' C-m
